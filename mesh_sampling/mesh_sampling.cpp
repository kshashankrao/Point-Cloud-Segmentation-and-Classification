...
main (int argc, char **argv)
{
  ...
  //cout <<"Output: "<<argv[ply_file_indices[0]]<<endl;
  std::ifstream infile("train.txt");
  std::string line;
  while (std::getline(infile, line)) {
    std::cout << line << std::endl;
    cout <<"Output: ply_file_indices[0] "<<argv[ply_file_indices[0]]<<endl;
      vtkSmartPointer<vtkPolyData> polydata1 = vtkSmartPointer<vtkPolyData>::New ();
      if (ply_file_indices.size () == 1)
      {
        pcl::PolygonMesh mesh;
        pcl::io::loadPolygonFilePLY (line, mesh);
      ...
      else // !write_normals && !write_colors
      {
        ...
        savePCDFileASCII ("person_"+str+".pcd", *cloud_xyz);
      }
      i++;
